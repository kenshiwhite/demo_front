...
export default function BusinessDirectoryScreen({ isSupplier }) {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            if (isSupplier) jobs.push(client.get('/api/auth/workers/'));
            const result = await Promise.all(jobs);
            setClients(result[0].data.results || result[0].data);
            setProducts(result[1].data.results || result[1].data);
            if (isSupplier) setWorkers(result[2].data.results || result[2].data);
        } catch (error) {
    ...
    const createRequest = async () => {
        ...
        try {
            ...
            Alert.alert('Заявка создана', `Заявка для ${requestClient.username} сохранена.`);
            load();
        } catch (error) {
    ...
    const renderClient = ({ item }) => (
        <View style={styles.card}>
            ...
            <TouchableOpacity style={styles.requestButton} onPress={() => openRequest(item)}>
                <Icon name="plus" size={15} color={colors.primary} />
                <Text style={styles.requestButtonText}>Создать заявку</Text>
            </TouchableOpacity>
        </View>
    );
    ...
    const renderWorker = ({ item }) => (
        <View style={styles.card}>
            ...
            {item.email ? <Text style={styles.detail}>Email: {item.email}</Text> : null}
            <Text style={styles.assigned}>Клиенты, добавленные сотрудником, закрепляются за ним.</Text>
        </View>
    );
    ...
    return <View style={styles.container}>
        ...
        <Modal visible={requestModal} transparent animationType="slide">
            <View style={styles.overlay}><KeyboardAvoidingView style={{ flex: 1, justifyContent: 'flex-end' }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}><View style={styles.modal}>
                <Text style={styles.modalTitle}>Заявка для {requestClient?.username}</Text><Text style={styles.muted}>Доступные остатки вашей компании</Text>
